...
The Account Manager is the sole entry point for all trader operations: account creation, collateral deposit and withdrawal, borrow, repay, liquidation, and `execute()`. It resolves all peer contract addresses from the Registry at runtime. No trader call bypasses it.
...
Three isolated pools: one each for XLM, USDC, and EURC. Each pool accepts LP deposits, lends to Margin Accounts, accrues interest via the Rate Model, and mints or burns its corresponding vToken (vXLM, vUSDC, vEURC). Pool isolation means a borrow shortfall in one asset does not spill into another.
...
<Steps>
  ...
  <Step title="Risk Engine evaluates post-operation health">
    Before any tokens move, the Account Manager calls the Risk Engine. It computes what the health factor would be after the operation completes. If it falls below 1.1x, the call panics here — nothing else executes.
  </Step>
...
| LendingPool (EURC) | Markets | EURC lending and borrowing |
| vXLM / vUSDC / vEURC | Markets | LP receipt tokens |
