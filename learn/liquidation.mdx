...
This loop runs for every token in the account's borrowed token list: XLM, USDC, and EURC. All debt positions are closed in a single transaction - partial liquidation is not supported.
...
`sweep_to()` calls `remove_collateral_token_bal_internal()` for each collateral token. This function contains explicit handlers for XLM, USDC, and EURC, which transfer the actual token balances:
