...
```
reflector.lastprice(asset) → (price, timestamp)
```

Reflector returns the most recent price for the asset and the timestamp it was recorded at. Vanna then normalizes the price to its internal WAD (1e18) format.

**If Reflector has no price for an asset, the transaction fails immediately.** Vanna never uses a default, a fallback, or a stale cache. Either the price is available, or the operation is blocked.
...
- It detects Blend tracking tokens by their symbol
- It looks up the underlying asset (XLM, USDC, EURC)
- It prices the position using the underlying asset's oracle price

A bXLM position is worth the same as XLM for collateral purposes — the Blend yield accrual is tracked separately.
