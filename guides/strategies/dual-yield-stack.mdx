...
description: "Supply EURC to Vanna's Earn pool while simultaneously running an XLM rate carry in Margin — stacking two independent yield streams with no directional price exposure."
mode: "wide"
---
...
Supply EURC to Vanna's Earn pool for passive stablecoin yield. At the same time, deposit USDC as Margin collateral, borrow XLM, and supply the borrowed XLM to a Blend XLM single-asset pool. The two strategies run independently and in parallel — each with no directional price exposure — producing two separate yield streams from the same capital base.

## Why this is delta-neutral

**Earn leg (EURC):** EURC is a USD-pegged stablecoin. Supplying it to Earn carries no meaningful price risk.
...
Combined, neither leg has directional exposure. The strategy earns from two rate differentials — the EURC lending yield and the XLM carry spread — running simultaneously without either one affecting the other.

## How to execute

<Steps>
  <Step title="Supply EURC to Earn">
    Go to **Earn** → select the EURC pool → **Supply** your EURC. Your vTokens begin accruing yield immediately. No further action needed on this leg.
  </Step>
  ...
  <Step title="Deposit USDC as Margin collateral">
    Go to **Margin** → **Deposit & Borrow** → deposit USDC from your wallet. This backs the XLM borrow independently of the EURC Earn position.
  </Step>
...
| | |
|---|---|
| EURC supplied to Earn (Leg 1) | $500 EURC |
| EURC Earn APY | 8% → earns $40/year |
...
| **Total capital deployed** | **$900 ($500 EURC + $400 USDC)** |
...
- **To exit the Earn leg:** Go to **Earn** → **Withdraw** → redeem your vEURC tokens.
...
| Rate risk (Earn leg) | Low | EURC Earn APY fluctuates with pool utilization |
