...
1. Any external caller can call `AccountManager.liquidate(liquidator, smart_account)`
2. The liquidator pays all outstanding debt from **their own wallet** — token balances are pulled from the liquidator address
3. **All collateral** in the SmartAccount is transferred to the liquidator
4. The liquidator profits from the spread: at 1.1× collateralization, collateral received is worth at least 10% more than the debt paid

<Note>
The liquidator's profit is structural: accounts only become liquidatable when collateral ≥ 1.1× debt. A timely liquidation always yields a positive spread (before gas). Race conditions between bots are possible — the first confirmed transaction wins.
</Note>
...
async function liquidateAccount(
  ...
  // liquidate(liquidator, smart_account)
  // The liquidator (botAddress) must sign and will pay all debt, receiving all collateral
  const txHash = await invokeContract(
    botAddress,
    ACCOUNT_MANAGER,
    'liquidate',
    [
      nativeToScVal(botAddress, { type: 'address' }),    // liquidator
      nativeToScVal(smartAccount, { type: 'address' }),  // smart_account
    ],
  );
