...
**Testnet address:** `CAWVGDG3TMGU4DC7POLSMDSBAJNJ363MHYPEAQHBRKOCLQIFP6WDXPJF`
...
```rust
fn liquidate(env: Env, liquidator: Address, smart_account: Address) -> ()
```

Liquidates an undercollateralized margin account. The liquidator pays all outstanding debt from their own wallet and receives all remaining collateral from the SmartAccount in return.

**Authorization:** `liquidator.require_auth()` — the caller (liquidator) must sign the transaction. The account owner is not involved and does not need to sign. This is fully permissionless.

**Condition:** `RiskEngine.is_account_healthy(collateral_usd, debt_usd)` must return `false`. Panics if the account is healthy.

**What happens:**
1. Validates the account is unhealthy via RiskEngine
2. For each borrowed token: reads outstanding debt, transfers tokens from the **liquidator's wallet** to the LendingPool, calls `LendingPool.collect_from()` to clear the borrow shares
3. Calls `SmartAccount.sweep_to(liquidator)` to transfer **all remaining collateral to the liquidator**
4. Emits event

**Liquidation incentive:** The liquidator pays the exact outstanding debt and receives the full collateral balance. Because the account only becomes liquidatable at 1.1× collateralization, a healthy liquidation always yields at least 10% profit on the debt repaid (before gas).
...
- [SmartAccount](/developers/contracts/smart-account) — per-user contract that AccountManager writes to
- [RiskEngine](/developers/contracts/risk-engine) — guard functions called before every borrow, withdraw, and liquidate
- [LendingPools](/developers/contracts/lending-pools) — pools that issue and collect debt
- [Integrate Margin](/developers/guides/integrate-margin) — end-to-end guide for building margin account flows
- [Liquidation Bots](/developers/guides/liquidation-bots) — guide for monitoring accounts and calling liquidate()
- [User Guide: Margin Account](/guides/margin/overview) — user-facing explanation of margin account lifecycle
- [Core Concepts: Liquidation](/learn/liquidation) — protocol-level liquidation flow and settle_account details
